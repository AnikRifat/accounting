...
class EntryExportController extends Controller
{
    public function __invoke(Request $request): StreamedResponse
    {
        ...
        $query = JournalEntry::visibleTo($request->user())
            ...
            ->filter($request->only(['from', 'to', 'type', 'account', 'party', 'payer', 'status', 'search']))
            ->withOutstanding()
            ->with(['company:id,name,code', 'lines.account:id,code,name,type,is_cash,is_system', 'party:id,name', 'bill:id,number', 'payer:id,name', 'creator:id,name'])
        ...
        return response()->streamDownload(function () use ($query): void {
            ...
            fputcsv($output, [__('Number'), __('Date'), __('Company'), __('Type'), __('Party'), __('Category'), __('Payment method'),
                __('Total (BDT)'), __('Paid (BDT)'), __('Paid / received by'), __('Due (BDT)'), __('Due date'), __('Due status'), __('Bill number'),
            ...
            foreach ($query->lazy(500) as $entry) {
                ...
                fputcsv($output, array_map($this->cell(...), [
                    ...
                    $entry->party?->name, $entry->categoryAccount()?->label(), $method, Money::toInput($entry->amount), $paid, $entry->payer?->name,

...
class Index extends Component
{
    ...
    private const FILTERS = ['company', 'from', 'to', 'type', 'account', 'party', 'status', 'search'];
    ...
    {
        return array_filter(['company' => $this->company, 'from' => $this->from, 'to' => $this->to, 'type' => $this->type,
    ...
    public function render(): View
    {
        ...
        $query = JournalEntry::visibleTo($user)->filter($this->filters());
        ...
        $selected = $companies->firstWhere('id', (int) $this->company);
        ...
        return view('livewire.admin.entries.index', [
            ...
            'companies' => ['' => __('All companies')] + $companies->mapWithKeys(fn (Company $company): array => [$company->id => $company->name.' ('.$company->code.')'])->all(),
            'types' => ['' => __('All types')] + collect(EntryType::cases())->mapWithKeys(fn (EntryType $type): array => [$type->value => $type->label()])->all(),
            'accounts' => ['' => __('All accounts')] + ($selected ? Account::query()->where('company_id', $selected->id)->orderBy('code')->get()
                ->mapWithKeys(fn (Account $account): array => [$account->id => $account->label()])->all() : []),
            'parties' => ['' => __('All parties')] + ($selected ? Party::query()->where('company_id', $selected->id)->orderBy('name')->get(['id', 'name'])
                ->mapWithKeys(fn (Party $party): array => [$party->id => $party->name])->all() : []),
            'statuses' => ['' => __('Any status')] + collect(DueStatus::cases())->mapWithKeys(fn (DueStatus $status): array => [$status->value => $status->label()])->all(),
            'companySelected' => $selected !== null,

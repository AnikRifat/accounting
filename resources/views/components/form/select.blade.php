...
<div class="field">
    ...
    <div class="search-select" x-data="searchSelect" x-modelable="value" {{ $attributes->whereStartsWith(['wire:model', 'x-model']) }} x-on:focusout="closeOnFocusOut($event)" x-on:keydown.escape="close(true)">
        <button id="{{ $id }}" type="button" x-ref="trigger" {{ $attributes->whereDoesntStartWith(['wire:model', 'x-model'])->except('required')->merge(['class' => 'form-control search-select-trigger']) }} aria-haspopup="listbox" x-bind:aria-expanded="open" aria-controls="{{ $id }}-listbox" aria-labelledby="{{ $id }}-label {{ $id }}" aria-invalid="{{ $errors->has($name) ? 'true' : 'false' }}" aria-describedby="{{ $id }}-help {{ $id }}-error" x-on:mousedown.prevent x-on:click="toggle()" x-on:keydown.down.prevent="show()" x-on:keydown.up.prevent="show()">
            <span x-text="selectedLabel()"></span>
        </button>
        <div class="search-select-panel" x-show="open" x-cloak>

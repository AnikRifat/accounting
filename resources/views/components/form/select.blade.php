...
<div class="field">
    ...
    <div class="search-select" x-data="searchSelect" x-modelable="value" data-placeholder="{{ $placeholder ?? __('Select an option') }}" {{ $attributes->whereStartsWith(['wire:model', 'x-model']) }} x-on:focusout="closeOnFocusOut($event)" x-on:keydown.escape="if (open) { $event.stopPropagation(); close(true) }">
        ...
        <div class="search-select-panel" x-show="open" x-cloak x-transition:enter="menu-enter" x-transition:enter-start="menu-hidden">
            ...
            <ul id="{{ $id }}-listbox" class="search-select-list" role="listbox" aria-labelledby="{{ $id }}-label" x-ref="list" x-on:mousedown.prevent>

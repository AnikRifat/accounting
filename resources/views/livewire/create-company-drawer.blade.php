<div x-on:open-create-company.window="$wire.open = true">
    <x-drawer id="create-company" wire:model="open" submit="save" :title="__('New company')" :description="__('Creates the company with the default chart of accounts, categories and payment methods, then switches the header to it.')">
        ...
        <x-form.input name="code" :label="__('Short code')" wire:model="code" required maxlength="10" autocomplete="off" class="uppercase" :help="__('2–10 letters or digits, used in entry numbers (e.g. MTL-000001).')" />
        ...
        <x-slot:footer><x-button type="submit" wire:loading.attr="disabled" wire:target="save">{{ __('Create company') }}</x-button><x-button variant="ghost" x-on:click="open = false">{{ __('Cancel') }}</x-button></x-slot:footer>
    </x-drawer>
</div>

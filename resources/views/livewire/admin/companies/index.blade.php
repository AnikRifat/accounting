<div class="page">
    ...
    <x-card flush>
        <x-slot:toolbar>
            <x-toolbar>
                <x-form.input name="search" :label="__('Search by name or code')" wire:model.live.debounce.300ms="search" type="search" maxlength="100" :placeholder="__('Name or code…')" />
                <x-slot:actions><x-table.export :columns="$this->tableColumns()" /></x-slot:actions>
            </x-toolbar>
        </x-slot:toolbar>
        <x-table.bulk />
        <x-table :caption="__('Companies')">
            <x-slot:head><x-table.check-all :ids="$companies->pluck('id')->all()" /><th>{{ __('Company') }}</th><th>{{ __('Code') }}</th><th>{{ __('Phone') }}</th><th>{{ __('Status') }}</th><th class="actions-col"><span class="sr-only">{{ __('Actions') }}</span></th></x-slot:head>
            @forelse($companies as $company)
                <tr wire:key="company-{{ $company->id }}">
                    <x-table.check :value="$company->id" :label="$company->name" />

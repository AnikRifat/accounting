    ...
    <div class="panel stack">
        ...
        @else
            <div class="table-wrap"><table class="report-table"><caption class="sr-only">{{ __('Employee cost') }}</caption>
                <thead><tr><th scope="col">{{ __('Employee') }}</th>@if($consolidated)<th scope="col">{{ __('Company') }}</th>@endif<th scope="col" class="text-right">{{ __('Entries') }}</th><th scope="col" class="text-right">{{ __('Total') }}</th><th scope="col" class="text-right">{{ __('Paid') }}</th><th scope="col" class="text-right">{{ __('Outstanding') }}</th><th scope="col" class="no-print">{{ __('Transactions') }}</th></tr></thead>
                <tbody>
                    @foreach($rows as $row)
                        <tr wire:key="employee-party-{{ $row['party']->id }}">
                            ...
                            <td class="no-print">@can('entries.view')<a class="text-link" href="{{ route('admin.entries.index', ['party' => $row['party']->id, 'type' => 'expense', 'from' => $range[0], 'to' => $range[1]]) }}" aria-label="{{ __('Transactions of :name', ['name' => $row['party']->name]) }}" wire:navigate>{{ __('View') }}</a>@endcan</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot><tr class="grand-total-row"><th scope="row" colspan="{{ $consolidated ? 2 : 1 }}">{{ __('Total') }}</th><td class="text-right tabular-nums">{{ $rows->sum('count') }}</td>

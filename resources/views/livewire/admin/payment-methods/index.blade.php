<div class="page">
    ...
    @else
        ...
        @endif
    @endif
    <x-sheet :label="__('Payment method')">
        @if($this->sheetAction() === 'create')
            <livewire:admin.payment-methods.form :key="'sheet-'.$sheet" />
        @elseif($this->sheetAction() === 'edit')
            <livewire:admin.payment-methods.form :payment-method="\App\Models\Account::query()->whereIn('company_id', auth()->user()->accessibleCompanyIds())->findOrFail((int) $this->sheetArgument())" :key="'sheet-'.$sheet" />
        @endif
    </x-sheet>
</div>

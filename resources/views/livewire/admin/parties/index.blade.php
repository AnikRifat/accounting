            ...
            @forelse($parties as $party)
                <tr wire:key="party-{{ $party->id }}">
                    ...
                    <td><div class="row-actions">
                        ...
                        @else
                            @can('parties.update')<x-button variant="ghost" size="sm" icon="pencil" :href="route('admin.parties.edit', $party)" :navigate="false" wire:click.prevent="openSheet('edit:{{ $party->id }}')" :label="__('Edit :name', ['name' => $party->name])">{{ __('Edit') }}</x-button>@endcan @can('parties.delete')<x-button variant="ghost" size="sm" icon="trash" class="text-danger" x-on:click="$dispatch('open-delete', { kind: 'party', id: {{ $party->id }} })" :label="__('Delete :name', ['name' => $party->name])" />@endcan
                        @endif
                    </div></td>
                </tr>
            @empty
